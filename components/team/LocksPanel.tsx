  ...
  const body = (
    <div className="team-shell" style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--tm-text)' }}>🔑 Türschlösser</div>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain' }}>
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '14px 14px 40px' }}>
          ...
          {/* Eigener Personen-Code */}
          {data?.meinCode && (
            <div style={{ borderRadius: 14, background: 'var(--tm-navy)', color: '#fff', padding: '14px 16px', marginBottom: 16 }}>
              <div style={{ fontSize: 12, color: 'var(--tm-muted2)', fontWeight: 700, letterSpacing: '0.04em' }}>👤 DEIN ZUGANGS-CODE</div>
              ...
              <div style={{ fontSize: 12, color: 'var(--tm-muted)', marginTop: 3 }}>gilt auf {data.meinCode.listingIds.length} Wohnung(en)</div>
            </div>
          )}
          ...
          {(data?.wohnungen ?? []).map((w) => {
            return (
              <div key={w.listingId} style={{ borderRadius: 14, background: 'var(--tm-card)', boxShadow: '0 0 0 0.5px var(--tm-line)', marginBottom: 12, overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px 8px', display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--tm-text)' }}>{w.title}</span>
                  {w.guestStatus === 'current' && <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--tm-green)' }}>🟢 belegt</span>}
                  {w.guestStatus === 'upcoming' && <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--tm-muted)' }}>Anreise {fmtRange(w.guestFrom, null)}</span>}
                </div>

                {/* Gäste-Türcode */}
                {w.guestCode ? (
                  <div style={{ padding: '0 16px 10px', fontSize: 13, color: 'var(--tm-text)' }}>
                    🔑 Gäste-Code <b style={{ fontVariantNumeric: 'tabular-nums', letterSpacing: '0.06em', color: 'var(--tm-text)' }}>{w.guestCode}</b>
                    {w.guestName && <span style={{ color: 'var(--tm-muted)' }}> · {w.guestName} · {fmtRange(w.guestFrom, w.guestTo)}</span>}
                  </div>
                ) : (
                  <div style={{ padding: '0 16px 10px', fontSize: 12.5, color: 'var(--tm-muted2)' }}>Kein aktiver Gäste-Code</div>
                )}
                ...
                {w.locks.map((lock) => {
                  ...
                  return (
                    <div key={key} style={{ padding: '10px 16px', boxShadow: 'inset 0 0.5px 0 var(--tm-line)', display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ flex: 1, minWidth: 0, fontSize: 14, color: 'var(--tm-text)', display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
                        <span>🚪 {lock.label}</span>
                        {/* §265: Akku + Online-Status */}
                        {h?.online === false && (
                          <span style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--tm-red)', background: 'var(--tm-red-soft)', borderRadius: 999, padding: '2px 8px' }}>⚠️ offline</span>
                        )}
                        {battSchwach ? (
                          <span title={h?.keypadCritical ? 'Keypad-Batterie kritisch' : undefined} style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--tm-yellow)', background: 'var(--tm-yellow-soft)', borderRadius: 999, padding: '2px 8px' }}>
                            🪫 {h?.battery !== null && h?.battery !== undefined ? `${h.battery} %` : 'schwach'}{h?.keypadCritical ? ' · Keypad' : ''}
                          </span>
                        ) : h?.battery !== null && h?.battery !== undefined ? (
                          <span style={{ fontSize: 10.5, fontWeight: 600, color: 'var(--tm-muted)' }}>🔋 {h.battery} %</span>
                        ) : null}
                      </span>
                      {!armed ? (
                        <button
                          ...
                          style={{
                            ...
                            background: busy ? 'var(--tm-surface2)' : 'var(--tm-navy)', color: busy ? 'var(--tm-muted)' : '#fff', cursor: busy ? 'default' : 'pointer',
                          }}>
                          {busy ? 'Öffne…' : 'Aufschließen'}
                        </button>
                      ) : (
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button onClick={() => setConfirmKey(null)} style={{ border: '1px solid var(--tm-line)', borderRadius: 999, padding: '8px 12px', fontSize: 13, background: 'var(--tm-card)', color: 'var(--tm-muted)', cursor: 'pointer' }}>Abbrechen</button>
          ...
          {!loading && !error && (data?.wohnungen ?? []).length === 0 && (
            <div style={{ textAlign: 'center', color: 'var(--tm-muted)', fontSize: 14, padding: '30px 0' }}>
          ...
          {(data?.protokoll ?? []).length > 0 && (
            <div style={{ marginTop: 22 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--tm-muted)', letterSpacing: '0.05em', margin: '0 4px 8px' }}>PROTOKOLL</div>
              <div style={{ borderRadius: 14, background: 'var(--tm-card)', boxShadow: '0 0 0 0.5px var(--tm-line)', overflow: 'hidden' }}>
                {(data?.protokoll ?? []).map((e, i) => (
                  <div key={i} style={{ padding: '9px 14px', fontSize: 12.5, color: 'var(--tm-text)', boxShadow: i ? 'inset 0 0.5px 0 var(--tm-line)' : 'none' }}>
                    <span style={{ color: 'var(--tm-muted)' }}>{fmtWhen(e.at)}</span>{' — '}
          ...
          <div style={{ marginTop: 18, fontSize: 11.5, color: 'var(--tm-muted2)', lineHeight: 1.5, padding: '0 4px' }}>
      ...
      {toast && (
        <div style={{
          ...
          background: 'var(--tm-navy)', color: '#fff', padding: '11px 18px', borderRadius: 999, fontSize: 14, fontWeight: 600,
