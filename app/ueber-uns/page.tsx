...
const STORY_2A = 'Aus der ersten gemeinsam renovierten Wohnung wurde schnell mehr. Heute betreiben wir als TRIMOSA Apartments & Homes eine wachsende Familie von Ferienwohnungen in Trier, Bitburg und der Südeifel — und erweitern sie Stück für Stück um'
const STORY_2_EM = 'immer neue Orte zum Ankommen, Wohlfühlen und Entdecken'
const STORY_2B = '. Als Nächstes: vier Apartments im denkmalgeschützten ehemaligen Weingut in'
const STORY_2C = 'und eine dritte Wohnung in Minden an der Sauer.'
const STORY_3 = 'Dabei bleibt alles in unserer Hand: Wir richten selbst ein, schreiben unsere Texte selbst, und wenn ihr bucht, bucht ihr direkt bei uns — ohne Vermittler, ohne versteckte Gebühren.'
...
export default async function UeberUnsPage() {
  ...
  const T = await makeTr(lang, lang === 'de' ? [] : [
    ...
    'Über TRIMOSA', 'Dein Zuhause auf Zeit.',
    HERO_TEXT, STORY_1, STORY_2A, STORY_2_EM, STORY_2B, STORY_2C, STORY_3,
    'Apartments', 'Regionen', '{n} Bewertungen', 'Direktbuchung',
    'Unser Name ist unsere Heimat',
    'Deutschlands älteste Stadt — unser Zuhause und Ausgangspunkt.',
    'Der Fluss, der die Region prägt — Weinberge, Radwege, Lebensgefühl.',
    'Die wilden Täler — Südeifel, Luxemburg-Grenze und großes Riesling-Terroir.',
    'Wie alles begann', 'Kanzem an der Saar',
    'Die drei hinter TRIMOSA', 'Gründer & Gastgeber', 'Unsere Regionen',
    'Region entdecken →',
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#F5F5F7' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 20px 60px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 'clamp(24px, 6vw, 64px)', flexWrap: 'wrap', margin: '0 0 44px', padding: '22px 20px', background: '#fff', borderRadius: '18px', border: '1px solid #EDE9DE' }}>
          {[
            { value: String(apartmentCount), label: T('Apartments') },
            { value: String(Object.keys(REGIONS).length), label: T('Regionen') },
            ...(avgScore ? [{ value: `★ ${avgScore}`, label: T('{n} Bewertungen').replace('{n}', String(totalCount)) }] : []),
            { value: '100 %', label: T('Direktbuchung') },
          ].map((s) => (
        ...
        <div style={{
          ...
          <p style={{ fontSize: '11px', fontWeight: 700, color: 'var(--gold)', letterSpacing: '0.12em', textTransform: 'uppercase', margin: '0 0 18px' }}>
            {T('Unser Name ist unsere Heimat')}
          </p>
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'stretch', gap: 'clamp(8px, 2.5vw, 18px)', flexWrap: 'wrap' }}>
            {[
              { letters: 'TRI', stands: 'Trier', text: T('Deutschlands älteste Stadt — unser Zuhause und Ausgangspunkt.') },
              { letters: 'MO', stands: 'Mosel', text: T('Der Fluss, der die Region prägt — Weinberge, Radwege, Lebensgefühl.') },
              { letters: 'SA', stands: 'Sauer & Saar', text: T('Die wilden Täler — Südeifel, Luxemburg-Grenze und großes Riesling-Terroir.') },
            ].map((b, i) => (
        ...
        <div style={{ maxWidth: '700px', margin: '0 auto 44px' }}>
          <h2 style={{ fontSize: '22px', fontWeight: 700, color: '#1A1400', margin: '0 0 12px', letterSpacing: '-0.01em' }}>{T('Wie alles begann')}</h2>
          <p style={{ fontSize: '15px', lineHeight: 1.8, color: '#3A3427', margin: '0 0 12px' }}>
            {T(STORY_1)}
          </p>
          <p style={{ fontSize: '15px', lineHeight: 1.8, color: '#3A3427', margin: '0 0 12px' }}>
            {T(STORY_2A)}{' '}
            <em>{T(STORY_2_EM)}</em>{T(STORY_2B)}{' '}
            <Link href="/region/saar" style={{ color: 'var(--gold-dark)', fontWeight: 600 }}>{T('Kanzem an der Saar')}</Link>{' '}
            {T(STORY_2C)}
          </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px', margin: '0 0 44px' }}>
          {VALUES.map((v) => (
            <div key={v.title} style={{ background: '#fff', borderRadius: '16px', border: '1px solid #EDE9DE', padding: '22px 20px' }}>
              ...
              <p style={{ fontSize: '15px', fontWeight: 700, color: '#1A1400', margin: '0 0 6px' }}>{T(v.title)}</p>
              <p style={{ fontSize: '13.5px', color: '#6B6455', margin: 0, lineHeight: 1.6 }}>{T(v.text)}</p>
            </div>
          ))}
        </div>

        {/* ── Founders ── */}
        <h2 style={{ fontSize: '22px', fontWeight: 700, color: '#1A1400', margin: '0 0 16px', letterSpacing: '-0.01em', textAlign: 'center' }}>{T('Die drei hinter TRIMOSA')}</h2>
        ...
        {/* ── Regions ── */}
        <h2 style={{ fontSize: '22px', fontWeight: 700, color: '#1A1400', margin: '0 0 14px', letterSpacing: '-0.01em', textAlign: 'center' }}>{T('Unsere Regionen')}</h2>
            ...
            return (
              <Link key={r.slug} href={`/region/${r.slug}`} className="listing-card" style={{ display: 'block', textDecoration: 'none', background: '#fff', borderRadius: '16px', border: '1px solid #EDE9DE', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '16px 18px 18px' }}>
                  ...
                  <p style={{ fontSize: '13px', color: '#6B6455', margin: '0 0 10px', lineHeight: 1.5 }}>{T(r.claim)}</p>
                  <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--gold-dark)' }}>{T('Region entdecken →')}</span>
                </div>
              </Link>
            )
          })}
        ...
        <p style={{ fontSize: '10.5px', color: '#AAA6A0', margin: '0 0 40px', textAlign: 'center' }}>
          {T('Regionsfotos: Wikimedia Commons — Urheber und Lizenz auf den jeweiligen Erlebnis-Seiten.')}
        </p>
