...
/* ── 1. Host Badge — the TRIMOSA host team (DB-driven), links to "Über uns" ── */
export interface HostBadgeMember { firstName: string; initials: string; avatarUrl: string | null }
const FALLBACK_TEAM: HostBadgeMember[] = [
  { firstName: 'Johannes', initials: 'JG', avatarUrl: null },
  { firstName: 'Pascal', initials: 'PJ', avatarUrl: null },
  { firstName: 'Dominik', initials: 'DP', avatarUrl: null },
]

export function HostBadge({ host, lang = 'de', team }: { host: HostProfile; lang?: UiLang; team?: HostBadgeMember[] }) {
  const members = team && team.length > 0 ? team : FALLBACK_TEAM
  const namesLine = members.length > 1
    ? `${members.slice(0, -1).map((m) => m.firstName).join(', ')} & ${members[members.length - 1].firstName}`
    : members[0]?.firstName ?? 'TRIMOSA'
  return (
    <Link href="/ueber-uns" title="Mehr über TRIMOSA erfahren" className="listing-card" style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 16px 8px 10px', borderRadius: '99px', backgroundColor: '#fff', border: '1px solid #E5E5EA', textDecoration: 'none', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', flexShrink: 0 }}>
        {members.map((m, i) => (
          <div key={m.initials + i} style={{
            ...
            {m.avatarUrl ? (
              <Image src={m.avatarUrl} alt="" fill sizes="32px" style={{ objectFit: 'cover' }} />
            ) : i === 0 && host.avatar_url ? (
      ...
      <div>
        <div style={{ fontWeight: 600, fontSize: '13px', color: '#1D1D1F', lineHeight: 1.2 }}>{namesLine}</div>
