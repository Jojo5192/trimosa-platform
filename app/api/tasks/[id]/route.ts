...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!auth.manage || !editable) {
    ...
    // Paragraph 303: die SELBST angelegte Aufgabe darf ihr Ersteller auch inhaltlich pflegen (Titel, Text,
    // Prio, Termin, Zuordnung) - Zuweisung/Sichtbarkeit bleiben Verwaltern vorbehalten
    if (!auth.manage && task.created_by === auth.userId) {
  ...
  if (typeof newAssignee === 'string' && newAssignee && newAssignee !== task.assignee_id && newAssignee !== auth.userId) {
    await sendPushToUser(newAssignee, '✅ Neue Aufgabe für dich', saved.title, '/team?tab=aufgaben', undefined, 'tasks')
