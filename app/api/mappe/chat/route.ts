...
export async function GET(req: NextRequest) {
  ...
  let out = rows
    ...
    // Interne Telefon-Einträge (§183/§189: Notizen UND aufgenommene
    // Telefonnachrichten) sind nur fürs Team — dem Gast nicht zeigen
    .filter((m) => !String(m.content ?? '').startsWith('☎️'))
    .map((m) => ({
